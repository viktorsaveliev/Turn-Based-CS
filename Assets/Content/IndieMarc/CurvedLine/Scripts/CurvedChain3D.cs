...
namespace IndieMarc.CurvedLine
{
    ...
    public class CurvedChain3D : MonoBehaviour
    {
        ...
        private void CalculateCurvedPath()
        {
            ...
            {
                if(p != null)
                    oripaths.Add(p.localPosition);
            }
            foreach (Transform p in paths)
            {
                if (p != null)
                    subpaths.Add(p.localPosition);
            }
        ...
        private void BuildMesh()
        {
            ...
            int max_meshes_count = max_meshes > 0 ? Mathf.Min(subpaths.Count, max_meshes) : subpaths.Count;
            float average_scale = (transform.lossyScale.x + transform.lossyScale.y + transform.lossyScale.z) / 3f;

            int nb_chains = Mathf.CeilToInt(total_dist / spacing);
            ...
            for (int i = 0; i < subpaths.Count; i++)
            {
                ...
                if (debug_chain)
                {
                    Debug.DrawRay(transform.TransformPoint(pos1), transform.TransformDirection(refdir) * 0.6f * average_scale, Color.red);
                }

                
                float chain_space = spacing * chain_index;
                ...
                if (dist_traveled >= chain_space && chain_index < nb_chains)
                {
                    ...
                    if (debug_chain)
                        Debug.DrawRay(transform.TransformPoint(ipos), transform.TransformDirection(refdir) * 0.5f * average_scale, Color.green);
                    ...
                    Vector3 pos_local = ipos;
                    Vector3 dir_local = dir.normalized;
            ...
            mesh.sharedMesh.vertices = (Vector3[]) vertices;
            mesh.sharedMesh.triangles = (int[])triangles;
            mesh.sharedMesh.normals = (Vector3[])normals;
            mesh.sharedMesh.uv = (Vector2[])uv;
        ...
        private void OnDrawGizmos()
        {
            ...
            {
                Vector3 prev = transform.TransformPoint(oripaths[0]);
                for (int i = 1; i < oripaths.Count; i++)
                {
                    Vector3 cur = transform.TransformPoint(oripaths[i]);
                    Debug.DrawLine(prev, cur, Color.blue);
                    prev = transform.TransformPoint(oripaths[i]);
                }
                Vector3 prev2 = transform.TransformPoint(subpaths[0]);
                for (int i = 1; i < subpaths.Count; i++)
                {
                    Vector3 cur = transform.TransformPoint(subpaths[i]);
                    Debug.DrawLine(prev2, cur, Color.white);
                    prev2 = transform.TransformPoint(subpaths[i]);
                }
            }
        }
        ...
        private bool IsInFrustum()
        {
            ...
            {
                Vector3 pos = transform.TransformPoint(p);
                Vector3 view_point = Camera.main.WorldToViewportPoint(pos);
                Vector3 view_point_dir = Camera.main.WorldToViewportPoint(pos + Vector3.up * frustum_cull_radius);
